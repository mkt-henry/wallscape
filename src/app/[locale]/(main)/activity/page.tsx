...
function NotificationItem({ notification }: { notification: NotificationWithActor }) {
  ...
  const getNotificationText = (type: NotificationType): string => {
    switch (type) {
      case 'like_post': return t('likePost')
      case 'like_comment': return t('likeComment')
      case 'comment': return t('commentPost')
      case 'reply': return t('replyComment')
      case 'follow': return t('follow')
      case 'mention': return t('mention')
      case 'nearby_post': return t('nearbyPost')
    ...
    <div className="flex flex-col min-h-screen">
      ...
      <div className="sticky top-0 z-20 bg-background/90 backdrop-blur-md pt-safe-top">
        <div className="flex items-center justify-between px-4 h-14">
          <h1 className="text-white text-xl font-black">{t('title')}</h1>
          {unreadCount > 0 && (
        ...
        <div className="flex px-4 pb-3 gap-2">
          {[
            ...
            { key: 'unread' as const, label: `${t('tabUnread')}${unreadCount > 0 ? `(${unreadCount})` : ''}` },
          ].map((tab) => (
            <button
              key={tab.key}
      ...
      <div className="flex-1">
        {!user && (
          <div className="py-20 flex flex-col items-center gap-4 px-6">
            ...
            <div className="text-center">
              <p className="text-white font-semibold mb-2">{t('loginRequired')}</p>
        ...
        {!isLoading && user && notifications?.length === 0 && (
          <div className="py-20 flex flex-col items-center gap-4">
            ...
            <div className="text-center">
              <p className="text-white font-semibold mb-1">{t('emptyAll')}</p>
