...
export async function GET(request: Request) {
  ...
  if (!apiKey || !botUserId) {
    return NextResponse.json(
      { error: 'Missing GNEWS_API_KEY or WALLSCAPE_BOT_USER_ID' },
      { status: 500 }
    )
  }
  ...
  try {
    ...
    if (!res.ok) {
      ...
      return NextResponse.json({ error: 'GNews API failed', status: res.status }, { status: 502 })
    }
    ...
    if (articles.length === 0) {
      return NextResponse.json({ inserted: 0, message: 'No articles found' })
    }
    ...
    if (newArticles.length === 0) {
      return NextResponse.json({ inserted: 0, message: 'All articles already exist' })
    }
